...
public class ScheduleService {
    ...
    // 机器人在线时间
    public static ConcurrentHashMap<String, LocalDateTime> robotAliveMap = new ConcurrentHashMap<>();
    ...
    // 检查机器人是否在线 ,心跳在5分钟内，算在线
    public boolean isAlive(String userName) {
        if (robotAliveMap.containsKey(userName)) {
            LocalDateTime now = LocalDateTime.now();
            LocalDateTime time = robotAliveMap.get(userName);
            if (time != null) {
    ...
    // 记录一次心跳时间
    public void alive(String userName) {
        robotAliveMap.put(userName, LocalDateTime.now());
    }
...
//    @Scheduled(cron = "0 0 9 * * ?")
    public void sendMsgToDriver() {
        ...
        listMap.forEach((vehicleNo, list) -> drivers.stream().filter(driver -> vehicleNo.equals(driver.getVehicleNo()) && StringUtils.hasText(driver.getWechat()))
                .findFirst()
                .ifPresent(driver -> {
                    // 把消息汇总成一条发送
    ...
    public void checkRobotIsAliveAndSendMsgToAdmin() {
        ...
        // 现在正在运行的机器人
        LocalDateTime now = LocalDateTime.now();
        long nowIsRunCount = robots.stream().filter(robot -> {
            if (robotAliveMap.containsKey(robot.getPhone())) {
                LocalDateTime time = robotAliveMap.get(robot.getPhone());
                Duration duration = Duration.between(now, time);
                long minutes = Math.abs(duration.toMinutes());//相差的分钟数
                return minutes <= 5;
            }
        ...
        // 如果需要运行的账号数量大于正在运行的账号数量，需要重启
        if (runRobotCount > nowIsRunCount) {
